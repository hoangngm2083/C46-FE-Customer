...
const useBookingService = ({
    ...
    const updateSlot = useMutation({
        mutationFn: ({ slotId, maxQuantity }: { slotId: string; maxQuantity: number }) =>
            axiosInstance.patch(`/slot/${slotId}`, { maxQuantity }).then(res => res.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['slots'] })
        }
    })
    ...
    const appointment = useQuery({
        ...
        queryFn: () => axiosInstance.get<IAppointmentDetails>(`/appointments/${appointmentId}`).then(res => res.data),
    ...
    const updateAppointmentState = useMutation({
        mutationFn: ({ appointmentId, state }: { appointmentId: string; state: AppointmentState }) =>
            axiosInstance.patch(`/appointments/${appointmentId}`, state).then(res => res.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['appointments'] })
    ...
    const deleteAppointment = useMutation({
        mutationFn: (appointmentId: string) => axiosInstance.delete(`/appointments/${appointmentId}`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['appointments'] })
            queryClient.invalidateQueries({ queryKey: ['slots'] })
        }
